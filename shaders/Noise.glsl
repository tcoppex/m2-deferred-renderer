              ...
              vec2 scaledTileRes = vec2(0.0f))
{
  ...
  */
  ...
  vec4 gx = 2.0f*fract(p * (1.0f / 41.0f)) - 1.0f ;
  vec4 gy = abs(gx) - 0.5f;  
  ...
  // 'Fast' normalization
  vec4 norm = inversesqrt(vec4(dot(g00, g00), dot(g10, g10), dot(g01, g01), dot(g11, g11)));
  g00 *= norm.x;  
  g10 *= norm.y;  
  g01 *= norm.z;  
  g11 *= norm.w;  
  ...
  vec4 fpt = fract(pt.xyxy) - vec4(0.0f, 0.0f, 1.0f, 1.0f);
              ...
              vec2 scaledTileRes = vec2(0.0f))
{
  ...
  */
  ...
  float k1 = n10 - n00;
  float k2 = n01 - n00;
  float k3 = n00 - n01 - n10 + n11;
  
  vec2 u = fade( fpt.xy );  
  output.x = k0 + k1*u.x + k2*u.y + k3*u.x*u.y;

  vec2 dpt = 30.0f*fpt.xy*fpt.xy*(fpt.xy*(fpt.xy-2.0f)+1.0f);
  
  output.y = dpt.x * (k1 + k3*u.y);
  output.z = dpt.y * (k2 + k3*u.x);
...
float fbm_dpnoise( vec2 pt, float zoom,
                   const int numOctave, 
                   const float frequency, 
                   const float amplitude)
{
...
float pnoise( vec3 pt,
              vec3 scaledTileRes = vec3(0.0f))
{
  // Retrieve the integral part (for indexation)
  vec3 ipt0 = floor(pt);
  ...
  // 'Fast' normalization
  vec4 norm = inversesqrt(vec4(dot(g000, g000), dot(g100, g100), dot(g010, g010), dot(g110, g110)));
  g000 *= norm.x;
  g100 *= norm.y;
  g010 *= norm.z;  
  g110 *= norm.w;  
  norm = inversesqrt(vec4(dot(g001, g001), dot(g101, g101), dot(g011, g011), dot(g111, g111)));
  g001 *= norm.x;
  g101 *= norm.y;
  g011 *= norm.z;  
  g111 *= norm.w;
