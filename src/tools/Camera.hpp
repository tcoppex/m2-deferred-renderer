 ...
 * 
 *                rethink it totally (remove the Singleton pattern)
 */
 
 
#ifndef TOOLS_TCAMERA_HPP
#define TOOLS_TCAMERA_HPP
...
class Camera : public Singleton<Camera>
{
  ...
  protected:
    ...
    float m_zNear;
    float m_zFar;    
    glm::vec2 m_linearizationParams;    /// used to linearize the depth buffer.
  ...
  public:
    ...
    void getProjectionParams( float *fov, float *zNear, float *zFar) const; 
    const glm::vec2& getLinearizationParams() const {return m_linearizationParams;}
    ...
    {
      m_viewMatrix=matrix;
      m_viewProjMatrix = m_projectionMatrix * matrix;
      
      m_worldMatrix = glm::inverse(m_viewMatrix);
      m_position = glm::vec3(m_worldMatrix[3]);
      m_direction = glm::vec3(-m_worldMatrix[2]);
