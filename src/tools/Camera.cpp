...
void Camera::setProjectionParams( float fov, float aspect, float zNear, float zFar)
{
  m_fov = fov;
  m_zNear = zNear;
  m_zFar = zFar;
  
  m_linearizationParams.x =  zFar / (zFar - zNear);
  m_linearizationParams.y = -zNear * m_linearizationParams.x;
...
void Camera::getProjectionParams( float *fov, float *zNear, float *zFar) const
{
  if (fov) *fov = m_fov;
  if (zNear) *zNear = m_zNear;
  if (zFar) *zFar = m_zFar;
}
...
void Camera::keyboardHandler(CameraKeys key, bool bPressed)
{
  ...
  // Setting it to true will set m_moveVelocity to the null vector
  //when the key will be released.
  m_bHasMoved = true;  
}

void Camera::motionHandler(int x, int y, bool bClicked)
{  
  if (bClicked)
  {
