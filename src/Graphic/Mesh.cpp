...
void PlaneMesh::draw(size_t numInstance) const
{
  assert( m_bInitialized );
  
  m_vertexBuffer.enable();
    glDrawArraysInstanced( GL_TRIANGLES, 0, m_count, numInstance);
 	m_vertexBuffer.disable();
}
...
void SphereMesh::init()
{
  ...
  m_count = 2*m_resolution*(m_resolution+2);  
  ...
  const float Delta = 1.0f / float(m_resolution);
...
void SphereMesh::draw(size_t numInstance) const
{
  assert( m_bInitialized );
  
  m_vertexBuffer.enable();
    glDrawArraysInstanced( GL_TRIANGLE_STRIP, 0, m_count, numInstance);
 	m_vertexBuffer.disable();
}
...
void DomeMesh::init()
{
  // BEWARE, triangles are in clockwise order.
  ...
  m_count = m_resolution*(m_resolution+2);  
  ...
  // Create the sphere from half bottom to top as a triangle_strip
  // (uv coordinates can also be computed in a shader)
	for (int j=0.5*m_resolution; j<m_resolution; ++j)
	{    
    ct = ct2;
    st = st2;
    ...
    *pNor = glm::vec3( ct, st, 0.0f);
    ...
    ++pNor; ++pPos; ++pUV;
      ...
      *pNor = glm::vec3( ct * cp, st, ct * sp);
      *pPos = m_radius * (*pNor);
      pUV->x = 0.5f * ct * cp;
			pUV->y = 0.5f * ct * sp;
      ++pNor; ++pPos; ++pUV;
      
      *pNor = glm::vec3( ct2 * cp, st2, ct2 * sp);
      *pPos = m_radius * (*pNor);
      pUV->x = 0.5f * ct2 * cp;
      pUV->y = 0.5f * ct2 * sp;
      ++pNor; ++pPos; ++pUV;
...
void DomeMesh::draw(size_t numInstance) const
{
  assert( m_bInitialized );
...
void ConeMesh::draw(size_t numInstance) const
{
  assert( m_bInitialized );
...
void CubeMesh::draw(size_t numInstance) const
{
  assert( m_bInitialized );
